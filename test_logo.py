...
class TestPrimitives(BaseUnitTest):
    ...
    def test_parentheses(self):
        ...
        self.check('show 1 + (3 * 8)', 'yield* prims.show(prims.sum(1, prims.product(3, 8)));')
        self.check('show 1 - (3 + 8)', 'yield* prims.show(prims.difference(1, prims.sum(3, 8)));')
        self.check('show (1 - (3 + 8))', 'yield* prims.show(prims.difference(1, prims.sum(3, 8)));')
        self.check('show 1 + (1 + 9) * 7', 'yield* prims.show(prims.sum(1, prims.product(prims.sum(1, 9), 7)));')
        self.check('show 1 + (1 + 9) - 7', 'yield* prims.show(prims.difference(prims.sum(1, prims.sum(1, 9)), 7));')
        self.check('or 1 = 1 2 = 2', 'prims.or(prims.equal(1, 1), prims.equal(2, 2))')
    ...
    def test_ask(self):
        self.check('ask [t1 t2 t3] [fd 50 rt 90 fd 50]',
                   'yield* prims.ask(["t1", "t2", "t3"], function*(){ let result = yield* prims.fd(50); '
                   'yield* prims.rt(90); yield* prims.fd(50);; return result });')

        self.check('ask "text2 [print "hello]',
                   'yield* prims.ask("text2", function*(){ let result = yield* prims.print("hello");; return result });')

    def test_unknown_type(self):
        self.check('show ask "P1_Wario [1 + 1]',
                   'yield* prims.show(yield* prims.ask("P1_Wario", function*(){ let result = prims.sum(1, 1); return result }));')

    def test_empty(self):
        self.check('ask [] []', 'yield* prims.ask("", function*(){ let result = ""; return result });')
    ...
    def test_if(self):
        ...
        self.check('ifelse colorunder = 15 [fd 50] [bk 50]',
                   ...
                   'function*(){ let result = yield* prims.fd(50);; return result }, '
                   'function*(){ let result = yield* prims.bk(50);; return result });')

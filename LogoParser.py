    ...
    def translate(self, root: Node) -> str:
        ...
        if token.type == TokenType.PRIMITIVE or token.type == TokenType.OPERATOR:
            ...
            if root.parent.value == 'LIST':
                parent_of_list_token = root.parent.parent.value
                parent_of_list_node = root.parent.parent
                if isinstance(parent_of_list_token, Token) and parent_of_list_token.type == TokenType.PRIMITIVE and \
                    parent_of_list_token.value.name in SPECIAL_USE_OF_LISTS:
                    PREV_NUM_OF_NODES = SPECIAL_USE_OF_LISTS[parent_of_list_token.value.name]
                    if parent_of_list_node.children.index(root.parent) >= PREV_NUM_OF_NODES:

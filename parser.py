...
def decode_key_value(content, decodeValue = True):
    """ FIX for "melody" "notes". read key first and content only after that """
    for i in range(0, len(content)-2, 2):
        if content[i] == 0 and content[i+1] == 0: #and content[i+2] == 0 and content[i+3] == 0:
            keyBytes = content[:i]
            contentBytes = content[i+2:len(content)]
            break
    logger.debug("keyBytes length: {}".format(len(keyBytes)))
    logger.debug("contentBytes length: {}".format(len(contentBytes)))
    key = keyBytes.decode('utf-16')
    logger.debug("mwx key: {}".format(key))
#    if key == "notes":
#        c = [key] + contentBytes.decode('ansi').split('\x00')
#   else:
    if decodeValue == True:
#        c = [key] + [contentBytes.decode('utf-16')]
        try:
...
def primary_process_object(object_content):
    ...
    """
    logger.debug("primary_process_object")
    ...
    while new_f.tell() < len(object_content):
        ...
        try:
            ...
            #FIX "notes" have ANSI encoded value and breaks decode
            #c = content[:30].decode('utf-16').split('\x00')
            c = decode_key_value(content, decodeValue = False)
        key = c[0]
        logger.debug("content length: {}".format(len(content)))
        if key != "notes":
            c = decode_key_value(content, decodeValue = True)
        if key == "name":
...
def primary_process_page(page, offset=10):
    ...
    while new_f.tell() < len(page):
        ...
        try:
            ...
            c = decode_key_value(content, decodeValue = False)
        key = c[0]
        logger.debug("project key: {}".format(key))
        if key == "name":
